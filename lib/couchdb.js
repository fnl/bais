var events = require('events'),
    ...
    url = require('url'),
    CouchClient, Server, Database, Design; // <- exports

/**
 * Create a new client for a CouchDB.
 *
 * In a nutshell, CouchClient works just as Node's HTTP client works:
 *
...
CouchClient = exports.CouchClient = function (url_string, headers) {
  ...
  if (headers) this.options.headers = headers;
};
...
/**
 * Return the base URL (schema, host, port) for this client, including
 ...
 * 
 * Examples:
 *   new_c = client.resource('database', 'doc id with / inside');
...
CouchClient.prototype.resource = function () {
  var url = this._resource.apply(this, Array.prototype.slice.call(arguments));
...
/**
 * Make a HEAD request (always raw requests).
 * 
 * @function
 * @param {String || Object} options or path
 * @param {Function} callback -> events.EventEmitter; optional
...
CouchClient.prototype.head = function (options, callback) {
  return this._doRequest('HEAD', options, null, callback, true);
};
...
exports.__test = {}; // need to expose some of them for testing (sandbox sux)
...
// TODO: do we want to have/need a caching mechanism here?
...
/**
 * A CouchClient with Server API-specific methods.
 *
 * @constructor
 */
Server = exports.Server = function Server() {};
Server.__proto__ = CouchClient;
Server.prototype.__proto__ = CouchClient.prototype;
 ...
 * @param {boolean} local append "/_local" to the final path; optional
 * @type Database
 */
Server.prototype.database = function (name, local) {
  if (local === true) name = [name, '_local'];
  return Database(this._resource(name), this.getHeaders());
};
...
/**
 * A CouchClient with Database API-specific methods.
 *
 * @constructor
 */
Database = exports.Database = function Database() {};
Database.__proto__ = CouchClient;
Database.prototype.__proto__ = CouchClient.prototype;
...
Database.prototype.changes = function (query, callback) {
  ...
  return this.get(options, (typeof query === 'function') ? query : callback);
};

/**
 * Create a new Design client for a design document.
 * 
 * @function
 * @param {String} ddoc design document ID
 * @type Design
 */
Database.prototype.design = function (ddoc) {
  return Desgin(this._resource('_design', ddoc), this.getHeaders());
};

/**
 * Create a new Design client for a view.
 * 
 * @function
 * @param {String} ddoc design document ID
 ...
 * @type Design
 */
Database.prototype.view = function (ddoc, view) {
  return Desgin(this._resource('_design', ddoc, '_view', view),
                this.getHeaders());
};

/**
 * Create a new Design client for a show.
 * 
 * @function
 * @param {String} ddoc design document ID
 ...
 * @type Design
 */
Database.prototype.show = function (ddoc, show) {
  return Desgin(this._resource('_design', ddoc, '_show', show),
                this.getHeaders());
};

/**
 * Create a new Design client for a list.
 * 
 * @function
 * @param {String} ddoc design document ID
 ...
 * @type Design
 */
Database.prototype.list = function (ddoc, list) {
  return Desgin(this._resource('_design', ddoc, '_list', list),
                this.getHeaders());
};

/**
 * Create a new Design client for a document update.
 * 
 * @function
 * @param {String} ddoc design document ID
 ...
 * @type Design
 */
Database.prototype.update = function (ddoc, update) {
  return Desgin(this._resource('_design', ddoc, '_update', update),
                this.getHeaders());
};


// SPECIALIZED DESIGN API

/**
 * A CouchClient with Design API-specific methods.
 *
 * @constructor
 */
Design = exports.Design = function Design() {};
Design.__proto__ = CouchClient;
Design.prototype.__proto__ = CouchClient.prototype;
